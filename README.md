...
We'll be working through the `csilvarivera/multi_tool_base_agent` repo, deploying to a shared GCP sandbox project (`ai-apps-sandbox`, region `europe-west1`).
...
* **Google Cloud CLI (gcloud)** — Install from [GCP CLI Installation Guide](https://cloud.google.com/sdk/docs/install). After installing, run `gcloud version` and confirm it's **≥ 480.0.0** so it has full workforce-pool support.
* **uv (recommended)** — Extremely fast Python package manager used by the training repo. Install with:
  ```bash
  curl -LsSf https://astral.sh/uv/install.sh | sh
  ```
  *(Plain pip + venv also works if you'd rather — see details below.)*

---

## 2. Sign In to GCP via Corporate (Azure) Workforce Identity
You won't use a personal `@gmail.com` or corporate Google account for this — instead, you'll authenticate through the same Azure AD login you already use, via a workforce identity pool.
...
> [!IMPORTANT]
> When the browser pops up, you should see your **Azure / Microsoft sign-in page**, not a Google account chooser. If you see "Choose an account" with `@gmail.com` accounts, the login config wasn't picked up — check the `--login-config` path and try again.
...
Open [.env](file:///Users/csilvarivera/Documents/multi_tool_base_agent/.env) (copied from [.env-example](file:///Users/csilvarivera/Documents/multi_tool_base_agent/.env-example)) in your editor and set the following values exactly:
...
Open the URL it prints (usually `http://localhost:8000`), pick the multi-tool agent, and ask it something like:
> "What's the weather in London? Then add 12 and 30 for me."

If you get a sensible reply, you're done — see you at the session!
...
| **Reauthentication required loop** | `gcloud` cached an old non-workforce session | Run `gcloud auth revoke --all`, then redo Step 2 |
...
| **gcloud: unrecognized argument: --login-config** | `gcloud` CLI is too old | Upgrade with `gcloud components update` (or reinstall from the link in Section 1) |
| **Behind the corporate VPN, browser auth hangs** | Proxy intercepts the OIDC redirect | Disconnect from the VPN for the login step only, then reconnect |

---

## What You'll Have at the End
1. A `gcloud` CLI session and ADC credentials on your laptop, both as your federated Azure identity, scoped to `ai-apps-sandbox`.
2. A clone of the training repo with a [.env](file:///Users/csilvarivera/Documents/multi_tool_base_agent/.env) pointing at the shared sandbox.
3. A working local ADK web playground talking to Vertex AI.
